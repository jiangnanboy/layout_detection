...
YOLOPredictor::YOLOPredictor(const string &modelPath,
                             const bool &isGPU,
                             float confThreshold,
                             float iouThreshold,
                             float maskThreshold) {
    ...
    if (num_output_nodes > 1) {
        this->hasMask = true;
        cout << "Instance Segmentation" << endl;
    } else
    ...
    for (int i = 0; i < num_input_nodes; i++) {
        ...
        input_names_ptr.push_back(move(input_name));
    ...
    for (int i = 0; i < num_output_nodes; i++) {
        ...
        output_names_ptr.push_back(move(output_name));

        Ort::TypeInfo outputTypeInfo = session.GetOutputTypeInfo(i);
        vector<int64_t> outputTensorShape = outputTypeInfo.GetTensorTypeAndShapeInfo().GetShape();
...
void YOLOPredictor::getBestClassInfo(vector<float>::iterator it,
                                     float &bestConf,
                                     int &bestClassId,
...
void YOLOPredictor::preprocessing(cv::Mat &image, float *&blob, vector<int64_t> &inputTensorShape)
{
...
vector<Yolov8Result> YOLOPredictor::postprocessing(const cv::Size &resizedImageShape,
    ...
    for (int i = 0; i < rows; i++) {
        vector<float> it(output0ptr + i * cols, output0ptr + (i + 1) * cols);
        float confidence;
        int classId;
        ...
        if (confidence > this->confThreshold) {
            if (this->hasMask) {
                vector<float> temp(it.begin() + 4 + classNums, it.end());
    ...
    if (this->hasMask) {
        ...
        vector<int> mask_protos_shape = {1, (int)this->outputShapes[1][1], (int)this->outputShapes[1][2], (int)this->outputShapes[1][3]};
        mask_protos = cv::Mat(mask_protos_shape, CV_32F, maskOutput);
    }

    vector<Yolov8Result> results;
    for (int idx : indices) {
        Yolov8Result res;
...
vector<Yolov8Result> YOLOPredictor::predict(cv::Mat &image)
{
    float *blob = nullptr;
    ...
    vector<Yolov8Result> result = this->postprocessing(resizedShape,
                                                            image.size(),
                                                            outputTensors);
    delete[] blob;
